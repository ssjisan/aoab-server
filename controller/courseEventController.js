...
import CourseEvent from "../model/courseEventModel.js";
...
export const listofAllCoursesEvents = async (req, res) => {
  try {
...
export const updateCoursesEventsSequence = async (req, res) => {
  try {
...
export const getFilteredCoursesEvents = async (req, res) => {
  try {
...
export const readCourseEvent = async (req, res) => {
  try {
    const { courseEventId } = req.params;
  ...
  try {
    ...
    if (coverPhoto) {
      ...
      try {
        ...
        courseEvent.coverPhoto = [{ url: uploadedImage.url, public_id: uploadedImage.public_id }];
      } catch (err) {
